...
## v4.6.4 (2026-08-11) — 性能优化与发布流程加固

### 性能优化
- **数据库与会话**：复用 SQLAlchemy 引擎，补充 SQLite 连接参数与索引，减少重复初始化和高频查询开销。
- **列表与对话加载**：服务端改为聚合统计、批量加载和分页查询，前端减少重复请求与无效渲染。
- **搜索与向量处理**：缓存搜索会话和向量模型，控制并发及结果规模，降低启动、检索和内存成本。

### 发布工程
- **Windows 构建链**：统一版本检查、依赖准备、sidecar/Tauri 构建和产物收集流程。
- **自动更新签名**：构建时校验 updater 公钥，使用私钥签署 MSI/NSIS 安装包，并生成 `latest.json`。
- **质量检查**：新增项目一致性、构建产物、更新清单和性能回归测试。
...
### 改进
- **隐藏"手动创建"卡片类型**: IDEA 库仅保留文献导入、AI 对话、随手记、网页抓取四种类型
- **分类卡片预览**: 每个分类卡片下方展示该类型最近 3 张卡片标题，可点击直接跳转详情
- **随手记重新编辑**: 随手记详情视图新增"编辑"按钮，支持修改标题和想法内容

### 待解决
- **校园网 PDF 下载**: 北航 WebVPN (e.buaa.edu.cn) 需浏览器交互式认证，程序化 HTTP 请求无法直接使用；当前代理设置仅对 HTTP 代理和 EasyConnect VPN 有效。CARSI 联邦认证方案待调研实现。

### 文件变更
- `nexus-ui/src/pages/KnowledgePage.tsx`: 移除手动创建、分类卡片预览、随手记编辑功能
...
- **Reader 翻页居中**: 修复 `p-8` padding 导致 scrollWidth 比 containerW 小 64px 的问题，内容宽度补偿 `+64px`；ResizeObserver 监听窗口变化自动重算
- **Reader 窗口自适应**: 新增 ResizeObserver 监听容器大小变化，自动重新计算页数和列宽

### 五子棋 AI 增强
- **模式表评估**: 识别跳三/跳四/组合威胁（双三、四三叉），替代简单连续棋子计数，评估准确度大幅提升
- **杀手启发式**: 每层记录 2 个引发 beta 截断的走法，优先搜索，显著提升剪枝效率
- **走法排序**: TT 走法 > 杀手走法 > 历史启发，搜索效率提升约 2 倍
- **开局库**: 中心优先策略，前 3 步使用开局知识
- **候选走法上限**: 从 22 提升至 30

## v4.5.3 (2026-06-30) — EPUB 翻页修复

### Bug 修复
- **EPUB 翻页失效**: 修复章节内点击左右区域无法翻页的问题。之前误加 CSS `column-count: 2` 双栏布局导致内容横向溢出、容器 `scrollHeight` 无法计算，翻页引擎始终认为只有 1 页，现已恢复为单栏滚动翻页模型

## v4.5.2 (2026-06-29) — Dashboard 修复 + Reader 双栏阅读

### Bug 修复
- **活跃度热力图修复**: 修复 Dashboard 和归档页的热力图因查询空日期字符串而始终无数据的问题，新增 `/api/tasks/heatmap` 后端端点
- **Dashboard 趋势图修复**: 替换硬编码的 mock 数据为真实 API 数据；周几标签从固定数组改为基于实际日期动态生成
- **归档页文字截断**: 任务内容和日报条目从 `line-clamp-2` 改为 `line-clamp-4`，添加 `title` 属性支持悬停查看全文

### 改进
- **Reader 双栏跨页显示**: PDF 阅读器渲染双页跨版（两只 canvas 并排），翻页每次前进/后退 2 页；EPUB/TXT/MD 内容使用 CSS 双栏布局（`column-count: 2`）
- **阅读进度持久化**: 基于 localStorage 自动保存/恢复阅读位置（章节 + 页码），重新打开同一本书自动跳转
