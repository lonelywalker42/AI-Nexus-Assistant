...
# ── 辉光管颜色层（暖琥珀色） ─────────────────────────────────
_NIXIE_COLORS = [
    (QColor(232, 168, 76, 25), 14),   # 光晕（琥珀）
    (QColor(232, 168, 76, 50), 10),   # 中层
    (QColor(240, 190, 100, 110), 6),  # 灯丝（蜂蜜）
    (QColor(248, 210, 140, 190), 3),  # 热核
    (QColor(255, 230, 170, 255), 1),  # 核心（亮蜂蜜）
]
...
class ClockWidget(QWidget):
    ...
    def _paint_pomodoro(self, painter: QPainter):
        ...
        # 颜色根据状态（暖色调）
        if self._pomodoro_state == "work":
            main_color = QColor(224, 112, 96)   # 暖红 — 工作
            label_text = "工作中"
        elif self._pomodoro_state == "break":
            main_color = QColor(122, 196, 138)  # 暖绿 — 休息
            label_text = "休息中"
        else:
            main_color = QColor(232, 168, 76)   # 琥珀 — 空闲
            label_text = "番茄钟"
