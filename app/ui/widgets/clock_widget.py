...
# ── 辉光管颜色层（清新蓝绿色） ──────────────────────────────
_NIXIE_COLORS = [
    (QColor(59, 130, 246, 25), 14),   # 光晕（蓝）
    (QColor(59, 130, 246, 50), 10),   # 中层
    (QColor(96, 165, 250, 110), 6),   # 灯丝（亮蓝）
    (QColor(147, 197, 253, 190), 3),  # 热核
    (QColor(191, 219, 254, 255), 1),  # 核心（浅蓝）
]
...
class ClockWidget(QWidget):
    ...
    def _paint_pomodoro(self, painter: QPainter):
        ...
        # 颜色根据状态（清新色调）
        if self._pomodoro_state == "work":
            main_color = QColor(59, 130, 246)   # 蓝色 — 工作
            label_text = "工作中"
        elif self._pomodoro_state == "break":
            main_color = QColor(16, 185, 129)   # 绿色 — 休息
            label_text = "休息中"
        else:
            main_color = QColor(107, 114, 142)  # 灰蓝 — 空闲
            label_text = "番茄钟"
