  ...
  return (
    <>
      <aside className="w-52 h-full flex flex-col" style={{ background: 'var(--glass-bg)', backdropFilter: 'blur(12px)', borderRight: '1px solid var(--border-color)' }}>
          ...
          {GROUPS.map(group => {
            ...
            return (
              <div key={group.key} className="mb-1">
                ...
                {groupPages.map(page => (
                  <NavItem key={page.id} page={page} active={activePage === page.id} onClick={() => onNavigate(page.id)} disabled={group.key === "personal"} />
                ))}
              </div>
            );
          })}
        ...
        >
          <span className="text-[10px]" style={{ color: "var(--text-muted)" }}>v2.2.0</span>
        </div>
      </aside>
        ...
        >
          ...
          >
            <div className="text-center space-y-3">
              ...
              <p className="text-xs" style={{ color: "var(--text-muted)" }}>v2.2.0</p>
