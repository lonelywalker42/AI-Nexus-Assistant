...
- 文献历史记录删除 + 综述/选题自动保存到历史
- 知识库 PDF 导入改用原始字节流传输 (application/octet-stream)
- 历史记录数据截断限制从 5000 → 50000 字符
- 模型编辑保存改为 PUT 更新（不新建）
- 滚轮缩放时钟窗口（0.6x-2.5x）
...
- PDF 导入：FormData → base64 JSON → 原始字节流（最终方案）
...
## Phase 9: v1.2.0 ✅ (2026-06-14)

### 功能更新
- 历史记录数据不再截断（移除 50000 字符限制）
- 搜索结果历史精简存储（只保留必要字段，避免数据膨胀）
- AI综述/选题讨论自动保存到历史 + 手动「保存到历史」按钮
- 历史记录点击直接加载内容（综述/选题/搜索三种类型）
- 综述/选题 Markdown 渲染增强（支持代码块、表格、行内代码、链接、分割线）
- 版本号弹窗显示数据库大小

### Bug 修复
- 历史综述/选题点击后空白页：改用 useEffect 确保 tab 切换后内容正确设置
- 移除调试输出
...
## Phase 10: v1.3.0 ✅ (2026-06-14)

### 功能更新
- 待办日历 UI 重构：无色透明玻璃效果 (rgba(255,255,255,0.18) + blur(40px))
- 窗口比例调整为 3:2 (480×320)，更宽屏紧凑
- 字体改为 Inter (Google Fonts)，适配空间计算/玻璃态界面
- 深色字体 (#0f172a slate-900)，透明玻璃上高可读性
- SVG 图标替代 emoji (关闭按钮、空状态)
- VisionOS 风格设计语言 (20px 圆角、柔和阴影、微交互)
- 移除所有诊断日志输出 (println!/eprintln!)

### Bug 修复
- 重复日历窗口：AtomicBool 自旋锁防止并发创建
- 重复日历窗口：on_menu_event 从 do_create_clock 移至 setup 全局注册一次
- 重复日历窗口：close_todo 改用 destroy() 替代 close() 强制销毁
- 菜单事件处理器：合并托盘菜单和时钟右键菜单为统一全局处理器
